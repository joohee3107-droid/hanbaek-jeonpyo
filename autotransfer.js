...
// ── 관리비 계산 시트 실시간 연동 ──────────────────────────────────────────────
// "한백 사택관리비" 구글시트(관리비 계산 탭)를 실시간으로 읽어서 SATAEK_FEE_TABLE을
// 매번 새로 만든다. 시트에 새 달 영수증이 들어오면 코드를 안 고쳐도 자동으로 반영된다.
// 시트가 사람이 입력한 값이라 오타/빈칸이 있을 수 있으므로, 읽기에 실패하면 기존 값(fallback)을 유지한다.
// 시트는 "링크가 있는 모든 사용자 - 보기" 로 공유되어 있어야 브라우저에서 읽을 수 있다.
const SATAEK_SHEET_ID = '18ZjefQyJu4yWKEKnvPRGLKCQPLwq4ig2_t8xZ41f6MI';
const SATAEK_SHEET_GID = '1878461671';

// gviz는 탭 이름(sheet=) 대신 gid= 로도 특정 탭을 지정할 수 있다(한글 탭 이름 인코딩 문제를 피함).
function loadSheetJsonpByGid(spreadsheetId, gid) {
  return new Promise((resolve, reject) => {
    const cbName = '__gsheet_cb_' + jsonpCounter++;
    const timeoutId = setTimeout(() => {
      cleanup();
      reject(new Error('관리비 시트 응답이 없습니다 (인터넷/공유 설정 확인)'));
    }, 15000);
    const script = document.createElement('script');
    function cleanup() { clearTimeout(timeoutId); delete window[cbName]; script.remove(); }
    window[cbName] = (response) => {
      cleanup();
      if (!response || response.status === 'error') { reject(new Error('관리비 시트를 불러오지 못했습니다 (공유 설정 확인)')); return; }
      resolve(response.table);
    };
    script.src = `https://docs.google.com/spreadsheets/d/${spreadsheetId}/gviz/tq?tqx=out:json;responseHandler:${cbName}&gid=${gid}`;
    script.onerror = () => { cleanup(); reject(new Error('관리비 시트 로드 실패 (공유 설정 확인)')); };
    document.body.appendChild(script);
  });
}

// 헤더가 첫 행이 아닐 수도 있어서, "사택주소"+"전기요금"이 함께 있는 행을 헤더로 잡고 그 아래를 데이터로 본다.
function extractSataekHeaderAndRows(table) {
  const norm = (s) => String(s).replace(/\s/g, '');
  const looksLikeHeader = (arr) => arr.some((h) => norm(h) === '사택주소') && arr.some((h) => norm(h) === '전기요금');
  const headerFromCols = tableHeader(table);
  if (looksLikeHeader(headerFromCols)) return { header: headerFromCols, rows: tableToRows(table) };
  const all = tableToRows(table);
  const idx = all.findIndex((r) => looksLikeHeader(r));
  if (idx === -1) return { header: headerFromCols, rows: all };
  return { header: all[idx], rows: all.slice(idx + 1) };
}

// 시트 행을 { "건물-동-호": { "관리비"/"도시가스"/"전기요금"/"수도요금": { "YYYY-MM": [금액,...] } } } 로 변환.
// YYYY-MM은 "영수증 년도/월" 기준이며, 실제 통장 출금월과의 시차(보통 1개월)는 매칭 단계에서 ±1개월로 흡수한다.
function buildSataekTableFromSheet(header, rows) {
  const norm = (s) => String(s).replace(/\s/g, '');
  const find = (label) => header.findIndex((h) => norm(h) === label);
  const cY = find('영수증년도'), cM = find('영수증월'), cAddr = find('사택주소');
  const utilCols = { '관리비': find('관리비'), '도시가스': find('도시가스'), '전기요금': find('전기요금'), '수도요금': find('수도요금') };
  if (cAddr === -1 || cY === -1 || cM === -1) return null; // 예상한 열이 없으면 실패로 보고 기존 값 유지
  const table = {};
  for (const row of rows) {
    const key = normalizeSataekKey(row[cAddr]);
    if (!key) continue;
    const y = parseInt(row[cY], 10);
    const m = parseInt(row[cM], 10);
    if (!y || !m) continue;
    const ym = `${y}-${String(m).padStart(2, '0')}`;
    for (const util in utilCols) {
      const ci = utilCols[util];
      if (ci === -1) continue;
      const amt = Math.round(parseAmount(row[ci]));
      if (!amt) continue;
      if (!table[key]) table[key] = {};
      if (!table[key][util]) table[key][util] = {};
      if (!table[key][util][ym]) table[key][util][ym] = [];
      if (!table[key][util][ym].includes(amt)) table[key][util][ym].push(amt);
    }
  }
  return Object.keys(table).length ? table : null;
}

// 관리비 시트를 실시간으로 읽어 SATAEK_FEE_TABLE을 갱신. 실패하면 기존 값을 그대로 둔다.
async function refreshSataekFeeTable() {
  try {
    const table = await loadSheetJsonpByGid(SATAEK_SHEET_ID, SATAEK_SHEET_GID);
    const { header, rows } = extractSataekHeaderAndRows(table);
    const built = buildSataekTableFromSheet(header, rows);
    if (built) {
      SATAEK_FEE_TABLE = built;
      console.log('관리비 시트 연동 성공: 사택 ' + Object.keys(built).length + '곳');
    } else {
      console.warn('관리비 시트에서 예상한 열(사택 주소/영수증 년도/월)을 못 찾아 기존 값을 유지합니다.');
    }
  } catch (e) {
    console.warn('관리비 시트 실시간 연동 실패, 기존 값 유지:', e.message);
  }
}
...
async function loadAll() {
  ...
  try {
    ...
    state.transfers = await _fetchRecurringFromDB();
    // 관리비 계산 시트를 실시간으로 읽어 사택 요금표(SATAEK_FEE_TABLE)를 최신으로 갱신한다.
    await refreshSataekFeeTable();
...
(function () {
  ...
  // 이 브라우저(localStorage)에만 있는 자동이체 리스트를 공유 DB(recurring_items)로 올린다.
  // 예전에 이 브라우저에서 직접 입력해둔 항목은 공유 DB에 없어서 다른 PC에서 안 보이는데,
  // 이 버튼을 한 번 누르면 그 항목들을 공유 DB로 올려서 어디서든 보이게 한다.
  // 이미 올라간 항목(같은 id)은 건너뛰므로 여러 번 눌러도 중복되지 않는다.
  async function uploadRecurringToDB() {
    if (!_dbClient) { alert('공유 DB에 연결되어 있지 않아요. 인터넷 연결을 확인한 뒤 다시 시도해주세요.'); return; }
    var local;
    try { local = loadRecurringItems(); } catch (e) { local = []; }
    // 아주 예전 항목이라 id가 없으면 여기서 새로 붙여준다
    local = local.map(function (it) { return it.id ? it : Object.assign({ id: makeItemId() }, it); });
    if (!local.length) { alert('이 브라우저에 올릴 자동이체 리스트가 없어요.'); return; }
    if (!confirm('이 브라우저에 저장된 자동이체 리스트 ' + local.length + '개를 공유 DB로 올릴까요?\n(이미 올라간 항목은 자동으로 건너뜁니다. 한 번만 누르면 돼요.)')) return;
    var existingIds;
    try {
      var res = await _dbClient.from('recurring_items').select('data');
      if (res.error) throw res.error;
      existingIds = new Set((res.data || []).map(function (r) { return r.data && r.data.id; }));
    } catch (e) { alert('공유 DB를 확인하지 못했어요: ' + (e.message || e)); return; }
    var toInsert = local.filter(function (it) { return !existingIds.has(it.id); }).map(function (it) { return { data: it }; });
    if (!toInsert.length) { alert('이미 모두 공유 DB에 올라가 있어요. 새로 올릴 항목이 없어요.'); return; }
    var ins = await _dbClient.from('recurring_items').insert(toInsert);
    if (ins.error) { alert('올리기 실패: ' + ins.error.message); return; }
    alert(toInsert.length + '개를 공유 DB로 올렸어요! 이제 다른 브라우저·PC에서 열어도 보여요.\n화면을 새로고침할게요.');
    location.reload();
  }
  ...
  function mount() {
    ...
    bar.innerHTML = '<b>데이터 백업/복원</b>' +
      ...
      '<button class="btn primary" id="uploadRecurringBtn" type="button">자동이체 리스트 공유 DB로 올리기</button>' +
      '<span style="color:#4338ca;">이 브라우저의 자동이체 리스트를 공유 DB로 올려 어디서든 보이게 해요 (한 번만).</span>';
    var main = document.querySelector('main');
    if (main) main.insertBefore(bar, main.firstChild);
    else document.body.insertBefore(bar, document.body.firstChild);
    document.getElementById('backupExportBtn').onclick = exportBackup;
    document.getElementById('uploadRecurringBtn').onclick = uploadRecurringToDB;
